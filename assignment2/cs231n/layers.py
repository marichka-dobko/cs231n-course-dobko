  ...
  """
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            for k in range(F): # F - number of filters
...
def conv_backward_naive(dout, cache):
  ...
  Inputs:N = x[0]
    F = w[0]
    H = 1 + (x[2] + 2* pad - w[2])/conv_param
    W = 1 + (W + 2 * pad - w[3]) / stride
    out = (N, F, H, W)
  - dout: Upstream derivatives.
  - cache: A tuple of (x, w, b, conv_param) as in conv_forward_naive

  Returns a tuple of:
  - dx: Gradient with respect to xN = x[0]
    F = w[0]
    H = 1 + (x[2] + 2* pad - w[2])/conv_param
    W = 1 + (W + 2 * pad - w[3]) / stride
    out = (N, F, H, W)
  ...
  """
    ...
    for i in xrange(H_out):
        for j in xrange(W_out):
            ...
            dx[:, :, i * stride: i * stride + HH, j * stride: j * stride + WW] += temp_binary_mask * (dout[:, :, i, j])[:, :, None,
                                                                                         None]
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
